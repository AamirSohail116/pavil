...
interface HomeBookingSummaryProps {
    roomTypes: Room[];
    setShowSummary?: (show: boolean) => void; // Optional prop to control summary visibility
}
...
interface BookingItem {
    ...
    checkIn: string;
    checkOut: string;
    roomType?: string; // Optional based on your data structure
}
...
export function HomeBookingSummary({ roomTypes, setShowSummary }: HomeBookingSummaryProps) {
    ...
    if (!bookingData || bookingData.length === 0) {
        setShowSummary?.(false); // Hide summary if no booking data
        return null
    };
        ...
        <>
            <Card className="">
                <CardHeader className="flex flex-row items-center justify-between">
                    <CardTitle className="text-lg font-semibold leading-[28px]">
                        Booking Details
                    </CardTitle>
                ...
                <CardContent className="space-y-4">
                    {roomInstances.map((roomInstance) => {
                        const room = roomTypes.find(rt => rt.id === roomInstance.roomId);
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                        <h4 className="font-medium leading-[24px] text-[16px]">
                                            {room?.name || roomInstance.roomType}
                                        </h4>
                                        ...
                                        <div className="flex flex-1 items-center justify-between gap-2">
                                            ...
                                            <div className="flex items-center">
                                                <span className="font-medium">
                                                    MYR {roomInstance.price.toFixed(2)}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
            ...
            <Link href={"/reserve-booking"} className="mt-4 flex justify-center py-3 mx-1 text-[14px] font-bold leading-[21px] items-center bg-[#dedede] text-[#008ace] border border-[#008ace]">
                complete Booking
            </Link>
            <BookNowSheet
                open={bookNowOpen}
